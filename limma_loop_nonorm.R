...
for (i in varlist[,1]) {
  ...
  if (length(levels(testvar_)) <2) {
    ...
    #loop otus and remove if non-zero < threshold for at least one level?? have to do this manually before running
    ...
    v_OTU<-cpm(dge)
     ...
     if (n==1){
		...
		write.table(norm_table,paste(outfolder,"tmm.tab",sep=""),sep="\t")} #N.B. first var must have no missing values in order to get a complete table
