...
x <- read.table(args[1],sep="\t",header=TRUE,row.names=1,check.names=FALSE)

y <- read.table(args[2],sep="\t",header=TRUE,row.names=1,check.names=FALSE)
...
row.names(x) <- x$OTU_ID
x <- x[,-1]
row.names(y) <- y$SampleID
y <- y[,-1]
